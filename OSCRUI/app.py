...
class OSCRUI():
    ...
    def setup_left_sidebar_league(self):
        ...
        global_frame = self.create_frame(style='medium_frame', size_policy=SMINMIN)
        season_frame = self.create_frame(style='medium_frame', size_policy=SMINMIN)
        ...
        maps_tabber.addTab(global_frame, 'Global')
        maps_tabber.addTab(season_frame, 'Seasons')
        self.widgets.map_tabber = maps_tabber
        self.widgets.map_tab_frames.append(global_frame)
        self.widgets.map_tab_frames.append(season_frame)
        left_layout.addWidget(maps_tabber, stretch=1)

        all_layout = QVBoxLayout()
        all_layout.setContentsMargins(0, 0, 0, 0)
        all_layout.setSpacing(0)
        background_frame = self.create_frame(global_frame, 'light_frame', style_override={
        ...
        global_frame.setLayout(all_layout)

        favorites_layout = QVBoxLayout()
        favorites_layout.setContentsMargins(0, 0, 0, 0)
        favorites_layout.setSpacing(0)
        season_combo = self.create_combo_box(season_frame)
        season_combo.addItem('Seasons')
        favorites_layout.addWidget(season_combo)
        self.widgets.season_combo = season_combo
        background_frame = self.create_frame(season_frame, 'light_frame', style_override={
                'border-radius': self.theme['listbox']['border-radius'], 'margin-top': '@csp'},
                size_policy=SMINMIN)
        background_layout = QVBoxLayout()
        background_layout.setContentsMargins(0, 0, 0, 0)
        ...
        season_selector.addItems(self.settings.value('favorite_ladders', type=list))
        season_selector.itemClicked.connect(
        ...
        season_frame.setLayout(favorites_layout)

        map_label = self.create_label(
                'Favourite Maps:', 'label_heading', frame, {'margin-top': '@isp'})
        left_layout.addWidget(map_label)
        ...
        background_layout.setContentsMargins(0, 0, 0, 0)
        background_frame.setLayout(background_layout)
        favourite_selector = QListWidget(background_frame)
        favourite_selector.setStyleSheet(self.get_style_class('QListWidget', 'listbox'))
        favourite_selector.setFont(self.theme_font('listbox'))
        favourite_selector.setSizePolicy(SMIXMIN)
        self.widgets.favourite_ladder_selector = favourite_selector
        favourite_selector.currentTextChanged.connect(
                lambda new_text: self.slot_ladder(new_text))
        background_layout.addWidget(favourite_selector)
